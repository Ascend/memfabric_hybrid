...
TEST_F(TestSmem, two_card_shm_create_success)
{
    ...
    DT_FUZZ_START(seed, CAPI_FUZZ_COUNT, fuzzName, 0)
    {
        ...
        auto func = [](uint32_t rank, uint32_t rankCount) {
            ...
            ret = smem_shm_topology_can_reach(handle, (rank + 1) % rankCount, &reachInfo);
            EXPECT_EQ(ret, 0);
...
TEST_F(TestSmem, smem_log_set_level)
{
    char fuzzName[] = "test_smem_log_set_level";
    uint64_t seed = 0;
    DT_FUZZ_START(seed, CAPI_FUZZ_COUNT, fuzzName, 0)
    {
        int32_t logLevel = *(int32_t *)DT_SetGetS32(&g_Element[0], 0);
        smem_init(0);
        auto ret = smem_set_log_level(logLevel);
