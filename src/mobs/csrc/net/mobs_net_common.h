...
class NetLink;
class NetEngine;
using NetLinkPtr = MORef<NetLink>;
using NetEnginePtr = MORef<NetEngine>;
...
inline Result NetEngineOptions::ExtractIpPortFromUrl(const std::string &url, NetEngineOptions &option)
{
    ...
    MO_LOG_ERROR_RETURN_IT_IF_NOT_OK(result, "Failed to extract url " << url << ", which is invalid");
    ...
    if (!(validator1.Validate(ipStr + ":" + portStr))) {
        MO_LOG_ERROR("Invalid url " << url);
        return MO_INVALID_PARAM;
    }
