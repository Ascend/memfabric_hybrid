...
if [ "$OP" = "0" ]; then
  ...
  for param in "${params[@]}"; do
      bash test_zbal_gather.sh "$param"
  done
else
  # zbal
  for param in "${params[@]}"; do
      echo "zbal gather data_len:${param}" >> ../perf.txt
      dir_path="./profiling.zbal_${param}"
      fn_analyze_perf "${dir_path}" "ZBALGatherInner" >> ../perf.txt
  done

  # hccl
  for param in "${params[@]}"; do
      echo "hccl gather data_len:${param}" >> ../perf.txt
      dir_path="./profiling.hccl_${param}"
      fn_analyze_perf "${dir_path}" "hcom_gather" >> ../perf.txt
  done
fi