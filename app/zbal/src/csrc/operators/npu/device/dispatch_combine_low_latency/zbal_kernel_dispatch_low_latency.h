...
public:
    ...
    __aicore__ inline void Init(GM_ADDR metaAddr, GM_ADDR x, GM_ADDR expertIds,
                                GM_ADDR expandXOut, GM_ADDR dynamicScalesOut, GM_ADDR expandIdxOut,
                                GM_ADDR expertTokenNumsOut,
                                GM_ADDR sendCountsOut, GM_ADDR putOffset, GM_ADDR putOffsetStatus,
                                uint32_t rank, uint32_t numExperts, uint32_t bs,
                                uint32_t hidden, uint32_t topK, int64_t magicVal, TPipe *pipe);
...
__aicore__ inline void DispatchLowLatency<TemplateTypeFunc>::Init(
                                GM_ADDR metaAddr, GM_ADDR x, GM_ADDR expertIds,
                                GM_ADDR expandXOut, GM_ADDR dynamicScalesOut, GM_ADDR expandIdxOut,
                                GM_ADDR expertTokenNumsOut,
                                GM_ADDR sendCountsOut, GM_ADDR putOffset, GM_ADDR putOffsetStatus,
                                uint32_t rank, uint32_t numExperts, uint32_t bs,
...
template <TemplateTypeClass>
__aicore__ inline void DispatchLowLatency<TemplateTypeFunc>::SplitToCore(uint32_t curSendCnt,
...
template <TemplateTypeClass>
__aicore__ inline void DispatchLowLatency<TemplateTypeFunc>::CalTokenSendExpertCnt(uint32_t dstExpertId,
                                                                                           int32_t calCnt,
...
__aicore__ inline void DispatchLowLatency<TemplateTypeFunc>::SetLayoutStatus() {
    ...
    for (uint32_t targetRankId = startRankId; targetRankId < endRankId; targetRankId++) {
        ...
        DataCopyPad(remoteLayoutStatusTensor_[epRankId_ * FLOAT_32B_ALIGN],
            statusTensor_, layoutStatusCopyParams);
    }
...
__aicore__ inline void DispatchLowLatency<TemplateTypeFunc>::SendCountNotify()
{
    ...
    for (uint32_t targetRankId = startRankId; targetRankId < endRankId; targetRankId++) {
        ...
        while ((curVal < minFlagVal) || (curVal > maxFlagVal)) {
            DataCopy(layoutStatusFp32Tensor_, layoutStatusTensor_[targetRankId * FLOAT_32B_ALIGN],
                    intriParams);
            SyncFunc<AscendC::HardEvent::MTE2_S>();
        ...
        if (rank == epRankId_) {
            ...
            for (uint32_t localMoeIndex = 0; localMoeIndex < moeExpertNumPerRank_; ++localMoeIndex) {
                ...
                tokenSums = ((expertTokenNumsType_ == 0) ? tokenSums : 0)\
                            + (curMoeIndexCnt - preMoeIndexCnt) + gatherCount_;
...
template <TemplateTypeClass>
__aicore__ inline void DispatchLowLatency<TemplateTypeFunc>::ReorderRecvDataOutput(
...
__aicore__ inline void DispatchLowLatency<TemplateTypeFunc>::SendToMoeExpert()
{
    ...
    putOffsetAlignSize = Ceil(epWorldSize_ * moeExpertNum_ * sizeof(int32_t),
        UB_ALIGN) * UB_ALIGN;  // 4 * ranks * moeNum

    DataCopyExtParams putOffsetParams = {1U,
        static_cast<uint32_t>(epWorldSize_ * moeExpertNum_ * sizeof(uint32_t)), 0U, 0U, 0U};
    ...
    for (int32_t index = startTokenId; index < endTokenId; ++index) {
        ...
        dstWinGMTensor.SetGlobalBuffer((__gm__ ExpandXOutType *)(dstPtr +
            hOutSizeAlign_ * (dstExpertOffset + curExpertCnt)));

        if constexpr (DynamicQuant || StaticQuant) {
            auto dstScalePtr = scaleShareAddrLt_.GetValue(dstRankId);
            dstScaleGMTensor.SetGlobalBuffer((__gm__ float *)(dstScalePtr +
                sizeof(float) * (dstExpertOffset + curExpertCnt)));
            ...
            DataCopyPad(dstScaleGMTensor, xOutFp32Tensor[hOutSizeAlign_ / sizeof(float)], scaleCopyParams_); // 拷贝Scale
...
__aicore__ inline void DispatchLowLatency<TemplateTypeFunc>::SendToSharedExpert()
{
    ...
    for (uint32_t tokenIndex = startTokenId; tokenIndex < endTokenId; ++tokenIndex) {
        ...
        uint32_t preCnt = (moeOnShareRank + epRankId_) * axisBS_ / sharedExpertRankNum_ - epRankId_\
                            * axisBS_ / sharedExpertRankNum_;
        ...
        if constexpr (DynamicQuant || StaticQuant) {
            ...
            if (isShareExpertRankFlag_) {
                ...
                DataCopyPad(dstScaleGMTensor[tokenIndex], xOutFp32Tensor_[axisH_ / sizeof(float)],
                            dataCopyParamsFloat);
...
DispatchLowLatency<TemplateTypeFunc>::ReduceMaxInplace(const LocalTensor<float> &srcLocal, uint32_t count)
{
