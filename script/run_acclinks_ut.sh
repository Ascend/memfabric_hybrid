...
readonly BUILD_PATH="$PROJECT_FULL_PATH/test/build"
...
cmake -DCMAKE_BUILD_TYPE=DEBUG -DBUILD_TESTS=ON -DBUILD_OPEN_ABI=ON -DBUILD_ACC_LINK=ON -S . -B ${BUILD_PATH}
make install -j5 -C ${BUILD_PATH}

cd "$BUILD_PATH/bin" && ./test_acc_links --gtest_output=xml:gcover_report/test_detail.xml

mkdir -p "$COVERAGE_PATH"
cd "$OUTPUT_PATH"
lcov --d "$BUILD_PATH" --c --output-file "$COVERAGE_PATH"/coverage.info -rc lcov_branch_coverage=1 --rc lcov_excl_br_line="LCOV_EXCL_BR_LINE|ASSERT*|LOG_*"
lcov -e "$COVERAGE_PATH"/coverage.info "*/csrc/*" -o "$COVERAGE_PATH"/coverage.info --rc lcov_branch_coverage=1
